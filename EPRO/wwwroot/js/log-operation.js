...
function ReplaceControlsInContent(containerSelector, page_title) {
    ...
    try {
        clone.find('.bootstrap-duallistbox-container').replaceWith(function () {
            var res = '<ul>';
            $(this).find('select[id*="bootstrap-duallistbox-selected-list"] option').each(function (i, e) {
                res += '<li>' + $(e).text() + '</li>';
            });
            res += '</ul>';
            return res;
    ...
    try {
        ...
        clone.find('div.input-group-append').remove();
        clone.find('label.control-label').removeClass('io-req');
...
function testLO() {
    var _html = ReplaceControlsInContent($('#saveContainer'), 'test');
    ShowModalDialog(_html, '');
}
